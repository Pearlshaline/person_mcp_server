...
// LIST all persons
server.tool('list_persons', 'Get all persons from the database', {}, async () => {
...
server.tool('get_person', 'Get a single person by their ID', { id: z.number().describe('The person ID') }, async ({ id }) => {
...
// CREATE a person
server.tool('create_person', 'Create a new person record in the database', {
...
// UPDATE a person
server.tool('update_person', 'Update an existing person by their ID', {
...
// DELETE a person
server.tool('delete_person', 'Delete a person record by their ID', { id: z.number().describe('The person ID to delete') }, async ({ id }) => {
    const existing = await prisma.person.findUnique({ where: { id } });
    if (!existing) {
        return { content: [{ type: 'text', text: `Person with ID ${id} not found.` }] };
    }
...
// SEARCH persons
server.tool('search_persons', 'Search for persons by name or email', { query: z.string().describe('Search query — matches first name, last name, or email') }, async ({ query }) => {
